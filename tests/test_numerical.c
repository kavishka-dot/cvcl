...
#define RUN(name)  do { printf("  %-48s", #name); g_run++; name(); printf("PASS\n"); } while(0)
...
TEST(test_draw_line_horizontal_count) {
    ...
    for (cvcl_i32 x=0; x<64; x++) if (row[x*3]==255) count++;
    ASSERT_EQ(count, 64);
...
TEST(test_draw_rect_filled_count) {
    ...
    for (cvcl_i32 y=0; y<64; y++) {
        ...
        for (cvcl_i32 x=0; x<64; x++) if (row[x]==255) count++;
    }
...
TEST(test_png_roundtrip_exact) {
    ...
    cvcl_io_write_png_native(&src, "/tmp/cvcl_rt_test.png");
    cvcl_io_read_png_native(&dst, "/tmp/cvcl_rt_test.png", NULL);
