...
cvcl_result_t cvcl_image_create(cvcl_image_t          *img,
    ...
    CVCL_MEMSET(img->data, 0, buf_size);
    return CVCL_OK;
}
...
cvcl_result_t cvcl_image_copy(cvcl_image_t       *dst,
    ...
    for (cvcl_i32 y = 0; y < src->height; y++) {
        CVCL_MEMCPY(cvcl_image_row(dst, y), cvcl_image_row(src, y),
               (cvcl_size)row_bytes);
    }
    return CVCL_OK;
}
...
cvcl_result_t cvcl_image_zero(cvcl_image_t *img) {
    ...
    CVCL_MEMSET(img->data, 0, cvcl_image_buffer_size(img));
    return CVCL_OK;
}
