 ...
 *
 *  4. cvcl_fill -- single-channel uses memset (one instruction on
...
cvcl_result_t cvcl_fill(cvcl_image_t *img, cvcl_u8 value) {
    ...
    if (img->channels == 1) {
        ...
        memset(img->data, value, cvcl_image_buffer_size(img));
        return CVCL_OK;
    }
    ...
    for (cvcl_i32 y = 1; y < img->height; y++)
        memcpy(cvcl_image_row(img, y), row0, (cvcl_size)row_bytes);
...
cvcl_result_t cvcl_histogram(const cvcl_image_t *img, cvcl_u32 *hist) {
    ...
    memset(hist, 0, (cvcl_size)ch * 256 * sizeof(cvcl_u32));
