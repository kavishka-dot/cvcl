...
cvcl_result_t cvcl_connected_components(cvcl_cc_result_t      *result,
    ...
    CVCL_MEMSET(result->labels, 0, n * sizeof(cvcl_u16));

    /* Allocate union-find structures (max labels = n/2 + 1) */
    cvcl_size max_labels = n / 2 + 2;
    cvcl_i32 *parent = (cvcl_i32 *)cvcl_alloc(a, max_labels * sizeof(cvcl_i32));
    cvcl_i32 *rank   = (cvcl_i32 *)cvcl_alloc(a, max_labels * sizeof(cvcl_i32));
    if (!parent || !rank) {
        cvcl_free(a, parent); cvcl_free(a, rank);
        cvcl_free(a, result->labels);
        result->labels = NULL;
        return CVCL_ERR_ALLOC;
    }
    CVCL_MEMSET(rank, 0, max_labels * sizeof(cvcl_i32));
    ...
    cvcl_i32 *remap = (cvcl_i32 *)cvcl_alloc(a, (cvcl_size)next_label * sizeof(cvcl_i32));
    if (!remap) {
        cvcl_free(a, parent); cvcl_free(a, rank);
        cvcl_free(a, result->labels); result->labels = NULL;
        return CVCL_ERR_ALLOC;
    }
    CVCL_MEMSET(remap, 0, (cvcl_size)next_label * sizeof(cvcl_i32));
    ...
    cvcl_free(a, parent); cvcl_free(a, rank); cvcl_free(a, remap);
    return CVCL_OK;
}
...
cvcl_result_t cvcl_cc_bboxes(cvcl_rect_t          **boxes,
    ...
    const cvcl_allocator_t *a = alloc ? alloc : cvcl_default_allocator();
    cvcl_rect_t *bb = (cvcl_rect_t *)cvcl_alloc(a,
