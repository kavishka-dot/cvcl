...
static cvcl_result_t morph_rect(cvcl_image_t       * CVCL_RESTRICT dst,
    ...
    dq_entry_t *dq = (dq_entry_t *)cvcl_alloc(a, (cvcl_size)dq_cap * sizeof(dq_entry_t));
    if (!dq) return CVCL_ERR_ALLOC;

    /* Intermediate: contiguous w*h (stride=w) for efficient column access */
    cvcl_u8 *tmp = (cvcl_u8 *)cvcl_alloc(a, (cvcl_size)w * h);
    if (!tmp) { cvcl_free(a, dq); return CVCL_ERR_ALLOC; }
    ...
    cvcl_u8 *out = (cvcl_u8 *)cvcl_alloc(a, (cvcl_size)w * h);
    if (!out) { cvcl_free(a, tmp); cvcl_free(a, dq); return CVCL_ERR_ALLOC; }
    ...
    for (cvcl_i32 y = 0; y < h; y++)
        CVCL_MEMCPY(cvcl_image_row(dst, y), out + (cvcl_size)y * w, (cvcl_size)w);
