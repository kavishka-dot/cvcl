...
static void make_gauss_fp(cvcl_i32 *kfp, cvcl_i32 ksize, cvcl_f32 sigma) {
    ...
    for (cvcl_i32 i = 0; i < ksize; i++) {
        ...
        fk[i] = CVCL_EXPF(-0.5f * x * x / (sigma * sigma));
        sum  += fk[i];
    }
    cvcl_i32 fp_sum = 0;
    for (cvcl_i32 i = 0; i < ksize; i++) {
...
cvcl_result_t cvcl_blur_gaussian(cvcl_image_t       *dst,
    ...
    cvcl_image_t mid;
    CVCL_CHECK(cvcl_image_create(&mid, w, h, ch, CVCL_DEPTH_U8, a));
...
cvcl_result_t cvcl_blur_box(cvcl_image_t       *dst,
    ...
    cvcl_u32 *acc = (cvcl_u32 *)cvcl_alloc(a, (cvcl_size)n * sizeof(cvcl_u32));
    if (!acc) { cvcl_image_free(&mid, a); return CVCL_ERR_ALLOC; }
    ...
    /* ---- Vertical pass ---- */
    cvcl_u32 *cacc = (cvcl_u32 *)cvcl_alloc(a, (cvcl_size)n * sizeof(cvcl_u32));
    if (!cacc) { cvcl_image_free(&mid, a); return CVCL_ERR_ALLOC; }
    CVCL_MEMSET(cacc, 0, (cvcl_size)n * sizeof(cvcl_u32));
