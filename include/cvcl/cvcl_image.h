...
typedef struct {
    ...
    const cvcl_allocator_t *alloc; /**< Allocator for the image memory         */
} cvcl_image_t;
...
/** Returns the allocator associated with the image, or default if NULL. */
CVCL_INLINE const cvcl_allocator_t *cvcl_image_allocator(const cvcl_image_t *img) {
    return (img && img->alloc) ? img->alloc : cvcl_default_allocator();
}
